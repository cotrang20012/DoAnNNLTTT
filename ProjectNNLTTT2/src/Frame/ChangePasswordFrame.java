	...
	public ChangePasswordFrame() {
		...
		btnChangePass.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Global.nv.getPassword().equals(textCurPass.getText())) {
					if (textNewPass.getText().equals(textRepeatPass.getText()) && textNewPass.getText().trim().length() != 0) {
						Global.nv.setPassword(textNewPass.getText());
						if (nhanvienDAO.UpdateNV(Global.nv)) JOptionPane.showMessageDialog(null, "Cập nhật thành công");
